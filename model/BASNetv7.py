...
class BBnet(nn.Module):
    def __init__(self,in1_chs=4,in1_out=3,in2_chs=4,in2_out=3,seg = False):
...
class SBBnet(nn.Module):
    def __init__(self,in1_chs=4,in1_out=3,in2_chs=4,in2_out=3,seg = False):
        super(SBBnet,self).__init__()
        self.corase = BASNet(in1_chs,in1_out)
        self.refine = BASNet(in2_chs,in2_out, res = True)
        # self.refine = UnetGenerator (in2_chs,in2_out,8)
    ...
    def forward(self,x,input_copy=None,seg=None):
        ...
        cs_input = torch.cat((x,seg),1)
        # resize the cs_input to half size using F.interpolate

        cs_input =  F.interpolate(cs_input, scale_factor=0.5, mode='bilinear', align_corners=True)

        x = self.corase(cs_input)
